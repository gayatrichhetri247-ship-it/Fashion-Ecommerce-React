    ...
    <div className="w-full px-4 sm:px-6 py-6">
      ...
      <div className="space-y-4 md:hidden">
        {data?.orders?.map((order) => {
          const isPaid = order.products[0]?.paymentStatus === "COMPLETE";
          return (
            <div 
              key={order._id}
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <span
                  className={`inline-flex items-center shrink-0 rounded-full px-2.5 py-0.5 text-xs font-semibold uppercase ${
                    isPaid
                      ? "bg-green-50 text-green-700 border border-green-200"
                      : "bg-amber-50 text-amber-700 border border-amber-200"
                  }`}
                >
                  {order.products[0]?.paymentStatus || "Pending"}
                </span>
              </div>

              {/* Middle Row: Product List & Quantities Map */}
              <div className="pt-3 border-t border-gray-100">
                <div className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1.5">
                  Ordered Items
                </div>
                <div className="divide-y divide-gray-50 bg-gray-50/50 rounded-lg border border-gray-100 p-2 space-y-1.5 max-h-40 overflow-y-auto">
                  {order.products.map((item) => (
                    <div 
                      key={item.productId?._id}
                      className="flex justify-between items-center text-xs text-gray-700 pt-1.5 first:pt-0"
                    >
                  ...
                  <td className="whitespace-nowrap px-6 py-4">
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${
                        order.products[0]?.paymentStatus === "COMPLETE"
                          ? "bg-green-50 text-green-700 border border-green-200"
                          : "bg-amber-50 text-amber-700 border border-amber-200"
                      }`}
                    >
